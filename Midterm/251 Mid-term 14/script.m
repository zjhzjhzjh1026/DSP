...
xlabel('n');ylabel('x(n)');title('N=256 point time series x(n)');
...
plot(n,xn_q);
axis([0 length(xn)-1 -1.1 1.1]);
xlabel('n');ylabel('Q[x(n)]');title(['N=256 point quantized time series Q[x(n)] b=' num2str(b)]);

en=xn_q-xn;   %%change for dither
figure(3);
plot(n,en);
xlim([0 length(xn)-1]);
xlabel('n');ylabel('e(n)');title(['N=256 point eror series e(n) b=' num2str(b)]);
%% 2
